class RetwettsController < ApplicationController
  ...
  #
  def create
    retwetting = current_user.retwetts.create(retwett_params)
    if retwetting.valid? and retwetting.persisted?
      flash[:success] = "You retweted: #{retwetting.twet.content}"
    else
  ...
  #
  def resource
    @resource ||= current_user.retwetts.where(:id => params[:id])
  end
  ...
  #
  def retwett_params
    params.require(:retwett).permit(:twet_id)
  end
